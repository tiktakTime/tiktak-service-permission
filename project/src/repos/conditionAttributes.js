const model = require('../models/conditionAttributes')
const attributes = require('../models/attributes')
const icons = require('../models/icons')
model.sync({force: false})

model.belongsTo(attributes, {
    foreignKey: 'attributeId'
})

model.belongsTo(icons, {
    foreignKey: 'iconId'
})
...
exports.listWithIncludes = async (where, options) => {
    return await model.findAll({ where, ...options, include: [{ model: attributes, where: { isActive: true }, include: [{model: icons}] }]})
}
